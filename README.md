...
5. В дереві елемента нескінченного порядку знайдеться або він самий, або
...
6. Для кожного елемента нескінченного порядку існує слово $s \in \{1, 2, 3, 4\}^*$,
   таке що $s = xiiiiiii...iii.., \, x \in \{1, 2, 3, 4\}^k, \, k \in \mathbf{N}, \, i \in \{1, 2, 3, 4\}$, 
   орбіта якого є нескінченною, але в алгоритмі цей шлях дає цикл, який починається після $x$

   __В процесі.__ Спочатку була гіпотеза, що цикл в принципі виникає тільки на таких словах (з точністю 
   до підстановочної орбіти для цієї літери, тобто якщо слово має вигляд 1001, то в орбітах нулів знайдеться одиниця, яку алгоритм просто не розглядав), проте контрприклад - `gcgbcb`
   
![gcgbcb](./graphs/cycle_with_non_trivial_path.png)
