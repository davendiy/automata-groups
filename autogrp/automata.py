...
class AutomataGroup:
    ...
    def __call__(self, word) -> AutomataGroupElement:
        if isinstance(word, str):
            ...
            if word not in self._defined_els:     # Lempel-Ziv-like algorithm
                res = self._e
                word = as_tokens(word, self.alphabet, split=True)
                if self._lempel_ziv:
                    left_word = word
                    last_value = self._e
                    while left_word:
    ...
    def multiply(self, x1, x2) -> AutomataGroupElement:
        ...
        if not self.is_defined(res_name):
            ...
            for i in range(self.size):
                ...
                child_name = self._reduce_func(a_child.replace('e', '') + b_child.replace('e', ''))
    ...
    @classmethod
    def generate_adding_machine(cls, apply_reduce_func=False, force=False):
        name = 'G_add'
        ...
        a_inv = AutomataGroupElement('a^-1', permutation=Permutation([1, 0]), children=['a^-1', 'e'], is_atom=True)
        b_inv = AutomataGroupElement('b^-1', permutation=Permutation([0, 1]), children=['b^-1', 'a'], is_atom=True)
        if force and name in AutomataGroup.all_instances():
            AutomataGroup.clear_group(name)

        if apply_reduce_func:
            return AutomataGroup(name, [a, b, a_inv, b_inv], reduce_function=free_reduce)
        else:
            return AutomataGroup(name, [a, b, a_inv, b_inv], reduce_function=id_func)
    ...
    def generate_H3_conjugated(cls, apply_reduce_func=False, force=False):
        ...
        if force and 'H3' in cls.all_instances():
            cls.clear_group('H3')
        if apply_reduce_func:
            reduce_func = partial(reduce_repetitions, alphabet=['a', 'b', 'c'])
            return AutomataGroup('H3*', [_a, _b, _c], reduce_function=reduce_func)
        else:
            return AutomataGroup('H3*', [_a, _b, _c])
    ...
    def generate_H4(cls, apply_reduce_func=False, force=False):
        ...
        if apply_reduce_func:
            reduce_func = partial(reduce_repetitions, alphabet=list('abcdfg'))
